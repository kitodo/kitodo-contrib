...
# even if the saving of the messages is not described there, ...
# but to have something for a rollback ...
# this scripts tars the messages files
...
dirToTar=$kitodoPath/messages

if [ ! -d $dirToTar ]
then
	echo "Current messages directory not found here: "$kitodoPath
	exit -3
fi
...
echo "even if the saving of the messages is not described there, ..."
echo "but to have something for a rollback ..."
echo "this scripts tars the current messages from" $dirToTar
echo "to" $destDir
echo ""
